"""Training callbacks used by CycPept3D.
...
The callbacks follow a minimal interface where hooks are optional and called
by the Trainer. See each class for details about expected arguments for hooks.
"""
...
class BaseCallback(ABC):
    ...
    The Trainer calls these hooks at specific points during training,
    validation, and checkpointing. Subclasses can override any subset of
    methods.

    Notes
    -----
    - Hook signatures intentionally accept ``*args`` and ``**kwargs`` to avoid
      tight coupling with the Trainer implementation. Refer to concrete
      callbacks for the actual arguments used.
    """
    ...
    def on_fit_start(self, *args, **kwargs):
        ...
        ----------
        *args, **kwargs
            Forwarded from the Trainer. No specific contract enforced here.
        """

        pass

    def on_fit_end(self, *args, **kwargs):
        """Called once at the end of ``fit`` on all ranks."""
    ...
    def on_batch_start(self, *args, **kwargs):
        """Called at the start of each processed batch."""

        pass

    def on_validation_step(self, *args, **kwargs):
        """Called on each validation step with predictions and targets.

        Typically receives tensors such as ``input``, ``target``, and ``pred``.
        """

        pass

    def on_validation_end(self, *args, **kwargs):
        """Called once after a full validation epoch."""
    ...
    def on_checkpoint_load(self, *args, **kwargs):
        ...
        The provided dict usually contains keys such as ``state_dict``,
    ...
    def on_checkpoint_save(self, *args, **kwargs):
        """Called right before saving a checkpoint.

        Allows mutating the checkpoint dictionary, e.g., to add custom state.
        """
...
class EarlyStoppingCallback(BaseCallback):
    ...
    def on_validation_end(self, epoch_idx, metric, **kwargs):
        ...
        epoch_idx : int
            Current epoch index.
        metric : float or Tensor
            The monitored scalar metric for comparison.
        **kwargs
...
class AllMetricsCallback(BaseCallback):
    ...
    def on_validation_step(self, input, target, pred):
        ...
        ----------
        input : Any
            Optional input tensor(s) for the model; unused here.
        target : Tensor
            Ground-truth values, expected to be broadcastable to predictions.
        pred : Tensor
            Model predictions.
        """
    ...
    def on_validation_end(self, epoch=None, **kwargs):
        ...
        ----------
        epoch : int, optional
            Epoch index to associate with logged metrics.
        **kwargs
            Ignored.
        """
        ...
        logging.info(
            f"""
            {self.prefix} MAE: {mae:.4f}
            {self.prefix} RMSE: {rmse:.4f}
            {self.prefix} R²: {r2:.4f}
            {self.prefix} Pearson r: {pearson:.4f}
            """
        )
