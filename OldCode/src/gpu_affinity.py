"""CPU affinity helpers for multi-GPU training.

This module adapts NVIDIA's public gpu_affinity helpers to assign processes to
CPU cores that match the GPU/NUMA topology. Correct affinity often improves
training throughput and stability on multi-socket systems.

References
----------
Original source and license are included below.
"""
...
class Device:
    """Thin NVML-backed GPU device wrapper.

    Parameters
    ----------
    device_idx : int
        CUDA device index used to query NVML for CPU affinity and metadata.
    """
    ...
    def get_name(self):
        """Return the human-readable GPU name as reported by NVML."""
        return pynvml.nvmlDeviceGetName(self.handle)

    def get_uuid(self):
        """Return the GPU UUID as reported by NVML."""
        return pynvml.nvmlDeviceGetUUID(self.handle)

    def get_cpu_affinity(self, scope):
        """Return list of logical CPU cores associated with this GPU.

        Parameters
        ----------
        scope : {"socket", "node"}
            Scope for NVML affinity query.

        Returns
        -------
        list of int
            Logical CPU core indices.
        """
        if scope == "socket":
...
def get_thread_siblings_list():
    """
    Discover hyperthreading core pairs from sysfs.

    Returns
    -------
    list of tuple[int, int]
        Pairs of logical cores recognized as siblings.
    """
...
def build_thread_siblings_dict(siblings_list):
    """Build mapping from core id to its sibling pair.

    Parameters
    ----------
    siblings_list : list[tuple[int, int]]
        Output of :func:`get_thread_siblings_list`.

    Returns
    -------
    dict[int, tuple[int, int]]
        Mapping from logical core to its sibling pair.
    """
    siblings_dict = {}
...
def group_list_by_key(the_list, key):
    ...
    the_list : Sequence
        Input sequence.
    key : Callable
        Key function used for grouping.

    Returns
    -------
    list[tuple]
        Tuples of grouped elements.
    """
...
def ungroup_affinities(affinities, scope, cores, min_cores=1, max_cores=None):
    """Convert grouped affinities into a flat list of logical cores.

    Parameters
    ----------
    affinities : list
        Grouped affinities per GPU.
    scope : {"socket", "node"}
        Scope at which to group cores.
    cores : {"all_logical", "single_logical"}
        Whether to include HT siblings.
    min_cores : int, default=1
        Minimum required number of physical cores per process.
    max_cores : int or None, optional
        If provided, trim the list to this maximum length.

    Returns
    -------
    list[list[int]]
        Ungrouped logical core indices per GPU.
    """
    if scope == "socket":
...
def check_affinities(affinities):
    """Validate that every pair of core sets is either identical or disjoint.

    Parameters
    ----------
    affinities : list[list[int]]
        Candidate affinities per device.

    Raises
    ------
    RuntimeError
        If two core sets partially overlap.
    """
...
def get_affinities(nproc_per_node, scope, exclude_unavailable_cores=True):
    """Query NVML and return recommended CPU affinities per GPU.
    ...
    scope : {"socket", "node"}
        Scope for NVML affinity query.
    exclude_unavailable_cores : bool, default=True
        If True, intersect with the current process's allowed cores.

    Returns
    -------
    list[list[int]]
        Logical cores per GPU.
    """
...
def get_grouped_affinities(nproc_per_node, exclude_unavailable_cores=True):
    """Return grouped affinities per GPU with thread sibling awareness."""
    ...
    def siblings_key(x):
        """Key function mapping a core id to its sibling tuple."""
...
def set_all(gpu_id, nproc_per_node, scope, cores, min_cores, max_cores):
    """
    Assign all available physical CPU cores recommended by NVML for a GPU.
...
def set_single(gpu_id, nproc_per_node, scope, cores, min_cores=1, max_cores=1):
    """
    Assign the first available physical core from the recommended list.
...
def set_single_unique(gpu_id, nproc_per_node, scope, cores, min_cores=1, max_cores=1):
    """
    Assign a single unique available physical core per process.
...
def set_unique(
    ...
    """Assign a unique subset of physical cores to each process/GPU.
    ...
    scope : {"node", "socket"}
    cores : {"all_logical", "single_logical"}
    mode : {"unique_contiguous", "unique_interleaved"}
    min_cores, max_cores : int
    balanced : bool, default=True
        If True, try to assign the same number of physical cores to each GPU.
    """
    grouped_affinities = get_grouped_affinities(nproc_per_node)
...
def set_affinity(
    ...
    *,
    ...
    """
    Assign a CPU affinity that matches CPU-GPU
