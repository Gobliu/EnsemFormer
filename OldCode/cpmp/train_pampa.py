...
def main():
    ...
    parameter_combinations = list(
        itertools.product(repeat_list, h_list, N_list, N_dense_list, slope_list)
    )
    for repeat, h, N, N_dense, slope in parameter_combinations:
        ...
        batch_size = 32
        lr = 1e-3
        best_model_saver = f"{outdir}/repeat_{repeat}_{ff}_ig_{ig}_h{h}_N{N}_N_dense{N_dense}_slope{slope}_batch_size{batch_size}.best_weight.pth"
        ...
        for epoch in range(1, epochs + 1):
            ...
            test_loss, r2, mse, mae = evaluate(
                model, data_loader_test, criterion, lr, device
            )
            ...
            print(
                f"epoch={epoch}, train_loss={train_loss}, test_loss={test_loss}, r2={r2}, mse={mse}, mae={mae}, best_loss={best_loss}"
            )
